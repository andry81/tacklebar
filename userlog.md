...
## 2024.02.20:
* :new: new: :sparkles: _config, deploy, res, src: standalone unsubst drive menu items

## 2024.02.17:
* :new: new: :sparkles: _config, deploy, res, src: subst/unsubst drive menu items

## 2024.02.17:
* :new: new: :sparkles: res/images/notepad: new refined notepad menu item icons
...
## 2024.01.22:
* :new: new: :sparkles: deploy, src/scripts/scm/shell/shell_copy_by_list.bat: added support for shortcut (`*.lnk`) target paths copy

## 2024.01.10:
* :new: new: :sparkles: deploy, res: Main menu buttons for `View changelog file`, `View userlog file`, `Open log directory`, `Open saveload directory`

## 2024.01.10:
* :new: new: :sparkles: _install*: copy installation log directory at the end of installation
...
## 2024.01.07:
* :new: new: :sparkles: deploy, res, src: Git button menu with `Git Bash` item

## 2024.01.04:
* :new: new: :sparkles: deploy, res, src, tools: added Git button menu with `GitExtensions` and `gitcmd` items
* :new: new: :sparkles: _install*: added `GIT_SHELL_ROOT` variable detection as a standalone variant of the Bash shell for Git
* :new: new: :sparkles: _install*: added `GitExtensions` installation detection
...
## 2023.11.23:
* :new: new: :sparkles: src/scripts/scm/shell/shell_move_by_list.bat: added `ALLOW_TARGET_FILES_OVERWRITE_ON_DIRECTORY_MOVE` configuration variable to explicitly control files overwrite on a directory move (disabled by default)
...
## 2023.10.16:
* :new: new: :sparkles: _install*: cleanup old `.*_prev_install` directories by moving into `.uninstall/*_prev_install` subdirectories

## 2023.10.10:
* :new: new: :sparkles: src/scripts/notepad/notepad_edit_files_by_list.bat: added `-append` flag and implemented new append mode with open from a file list file
...
## 2023.09.17:
* :new: new: :sparkles: src/scripts/terminal, deploy/totalcmd/ButtonBars/terminal: run terminal shell without logging
...
## 2023.08.14:
* :new: new: :sparkles: src/scripts/scm/shell/shell_reset_links_in_dir.bat: added `-allow-auto-recover`, `-allow-target-path-reassign`, `-allow-wd-reassign`, `-reset-target-path-from-wd`, `-reset-target-path-from-desc`, `-reset-target-name-from-file-path`, `-reset-target-name-from-file-path` flags usage
* :new: new: :sparkles: deploy/totalcmd/ButtonBars/_common/link/link.bar: added `-allow-auto-recover + -allow-target-path-reassign`, `-allow-target-path-reassign`, `-reset-target-path-from-wd`, `-reset-target-path-from-desc`, `-reset-target-name-from-file-path`, `-reset-target-name-from-file-path` flag combinations usage
...
## 2023.06.11:
* :new: new: :sparkles: _install: copy `.externals` into installation directory to be able to identify external dependencies from the installation directory
...
## 2023.05.10:
* :new: new: :sparkles: src/scripts/scm/shell/shell_move_by_list.bat: added config file generation and edit for script parameterization
...
## 2023.03.16:
* :new: new: :sparkles: src/scripts/scm/shell/shell_reset_links_in_dir.bat: added `-p[rint-assign]` flag to print all assignments
* :new: new: :sparkles: src/scripts/scm/shell/shell_reset_links_in_dir.bat: added `-reset-wd` flag as shorter version of `-reset-wd-from-target-path`
...
* :new: new: :sparkles: deploy/totalcmd/ButtonBars/_common/link/link.bar: added usage of `shell_reset_links_in_dir.bat` with `-reset-wd-from-target-path` flag

## 2022.09.29:
* :new: new: :sparkles: _install.bat, deploy/totalcmd: added option to install a single button menu instead of multiple buttons menu
...
* :new: new: :sparkles: src/scripts/scm/shell: added `shell_reset_links_in_dir.bat` script to reset shortcut files in a directory
* :new: new: :sparkles: deploy/totalcmd/ButtonBars/_common/link/link.bar: shell reset shortcut files in current directory (recursively) item

## 2022.06.20:
* :wrench: fixed: src/scripts: potential `!` character truncation in path variables expansion

## 2022.03.14:
* :new: new: :sparkles: src/scripts/scm/shell: added `shell_mklink_by_list.bat` script to create shortcut files by list
* :new: new: :sparkles: deploy/totalcmd/ButtonBars/_common: mklink, _menu.bar: menu for `shell_mklink_by_list.bat` script
* :new: new: :sparkles: res/images/mklink: icons for `shell_mklink_by_list.bat` script
...
## 2021.10.06:
* :new: new: :sparkles: deploy: MinTTY shell terminal support as a standalone submenu (MinTTY button bars + icons)
## 2021.10.05:
* :new: new: :sparkles: src/scripts: `MINTTY*_ROOT`, `MINTTY*_TERMINAL_PREFIX`, `USE_MINTTY` variables to enable use mintty in scripts
* :new: new: :sparkles: src/scripts: `MSYS*_TERMINAL_PREFIX`, `CYGWIN*_TERMINAL_PREFIX` variables to use mintty terminal from msys/cygwin
...
## 2021.03.12:
* :new: new: :sparkles: deploy/totalcmd/ButtonBars/_common/saveload/load, deploy/totalcmd/Profile: `LOADLIST` command usage (available in the Total Commander beginning from version 10b1).
