...
if %WINDOWS_MAJOR_VER% GTR 5 (
  "%CONTOOLS_ROOT%/ToolAdaptors/lnk/cmd_admin.lnk" /C set "IMPL_MODE=1" ^& set "INIT_VARS_FILE=%PROJECT_LOG_DIR%\init.vars" ^& call "%?~dp0%._install\_install.update.terminal_params.bat" -update_screen_size -update_buffer_size ^& call "%?~f0%" %* 2^>^&1 ^| "%CONTOOLS_UTILITIES_BIN_ROOT%/ritchielawrence/mtee.exe" /E "%PROJECT_LOG_FILE:/=\%"
) else "%CONTOOLS_ROOT%/ToolAdaptors/vbs/winshell_call.vbs" -verb runas -unesc "%COMSPEC%" /C set "%%22IMPL_MODE=1%%22" ^& set "%%22INIT_VARS_FILE=%PROJECT_LOG_DIR%\init.vars%%22" ^& call "%%22%?~dp0%._install\_install.update.terminal_params.bat%%22" -update_screen_size -update_buffer_size ^& call "%%22%?~f0%%%22" %* 2^>^&1 ^| "%%22%CONTOOLS_UTILITIES_BIN_ROOT%/ritchielawrence/mtee.exe%%22" /E "%%22%PROJECT_LOG_FILE:/=\%%%22"
exit /b
