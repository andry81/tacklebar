...
(
  ...
  if %CONEMU_ENABLE%0 NEQ 0 if /i "%CONEMU_INTERACT_MODE%" == "run" (
    %CONEMU_CMDLINE_RUN_PREFIX% "%COMSPECLNK%" /C @%%?__CMDLINE__%% -cur_console:n 2^>^&1 ^| "%TACKLEBAR_RITCHIELAWRENCE_MTEE_ROOT%/mtee.exe" /E "%PROJECT_LOG_FILE:/=\%"
    call set LASTERROR=%%ERRORLEVEL%%
    set "CONTOOLS_ROOT=%CONTOOLS_ROOT%"
    set "FLAG_PAUSE_ON_ERROR=%FLAG_PAUSE_ON_ERROR%"
    goto IMPL_EXIT
  )
  "%COMSPECLNK%" /C @%%?__CMDLINE__%% 2>&1 | "%TACKLEBAR_RITCHIELAWRENCE_MTEE_ROOT%/mtee.exe" /E "%PROJECT_LOG_FILE:/=\%"
  call set LASTERROR=%%ERRORLEVEL%%
  set "CONTOOLS_ROOT=%CONTOOLS_ROOT%"
  set "FLAG_PAUSE_ON_ERROR=%FLAG_PAUSE_ON_ERROR%"
  goto IMPL_EXIT
