...
rem USAGE:
rem   shell_copy_by_list.bat [-+] <flags> [--] <current-directory> <list-file> [<destination-directory>]
...
) else set "COPY_FROM_LIST_FILE_TMP=%LIST_FILE_PATH%"
...
( "%SystemRoot%\System32\fc.exe" /B "%COPY_TO_LIST_FILE_TMP%" "%COPY_TO_LIST_FILE_EDITED_TMP%" >nul ) <nul && exit /b 0
...
if not "%TO_SHORTCUT_FILE_PATH%" == "%FROM_SHORTCUT_FILE_PATH%" (
  echo;%?~%: error: shortcut paths is not equal, skipped:
