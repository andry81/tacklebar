...
if %FROM_FILE_PATH_LONG% EQU 0 if %TO_FILE_PATH_LONG% EQU 0 (
  call :COPY_FILE /B /Y "%%FROM_FILE_PATH%%" "%%TO_FILE_PATH%%" || (
    ...
    if %TO_FILE_PATH_EXISTS% EQU 0 if defined TO_FILE_PATH del /F /Q /A:-D "%TO_FILE_PATH%" 2>nul
    exit /b 31
  )
  exit /b 0
)
...
rem CAUTION: must check on empty variable to avoid accidental `del /Q ""` case
if defined COPY_WITH_RENAME_DIR_TMP if defined TO_FILE_NAME del /F /Q /A:-D "%COPY_WITH_RENAME_DIR_TMP%\%TO_FILE_NAME%"
...
call :COPY_FILE /B /Y "%%FROM_FILE_PATH%%" "%%COPY_WITH_RENAME_DIR_TMP%%\%%TO_FILE_NAME%%" || (
  echo;%?~%: error: could not copy into temporary directory: "%FROM_FILE_PATH%" -^> "%COPY_WITH_RENAME_DIR_TMP%\%TO_FILE_NAME%".
  exit /b 50
) >&2
