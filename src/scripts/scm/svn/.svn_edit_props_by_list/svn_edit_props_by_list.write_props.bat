...
:UPDATE_PROPS
rem at first check if a property file is blank or contains only white spaces, and a property original file has a not white space character, then delete the property
...
( "%SystemRoot%\System32\fc.exe" /B "%PROP_VALUE_FILE%" "%PROP_VALUE_FILE%.orig" >nul ) <nul && exit /b 0
