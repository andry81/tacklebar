...
if %FLAG_CONVERT_FROM_UTF16% NEQ 0 (
  ...
  call "%%CONTOOLS_ROOT%%/encoding/ansi2any.bat" UTF-16LE UTF-8 "%%~1" > "%RENAME_FROM_LIST_FILE_TMP%"
) else if %FLAG_CONVERT_FROM_UTF16BE% NEQ 0 (
  call "%%CONTOOLS_ROOT%%/encoding/ansi2any.bat" UTF-16BE UTF-8 "%%~1" > "%RENAME_FROM_LIST_FILE_TMP%"
) else (
  set "RENAME_FROM_LIST_FILE_TMP=%~1"
)

call :COPY_FILE "%%RENAME_FROM_LIST_FILE_TMP%%" "%%PROJECT_LOG_DIR%%/%%RENAME_FROM_LIST_FILE_NAME_TMP%%"
call :COPY_FILE "%%RENAME_FROM_LIST_FILE_TMP%%" "%%PROJECT_LOG_DIR%%/%%RENAME_TO_LIST_FILE_NAME_TMP%%"
...
call :COPY_FILE "%%PROJECT_LOG_DIR%%/%%RENAME_TO_LIST_FILE_NAME_TMP%%" "%%RENAME_TO_LIST_FILE_TMP%%"
...
:SVN_RENAME_FILE
call :CMD svn rename "%%FROM_FILE_PATH%%" "%%TO_FILE_PATH%%" --non-interactive || exit /b
