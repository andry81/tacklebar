...
(
  ...
  "%CYGWIN_ROOT%\bin\bash.exe" -c "{ echo "">%CYGWIN_ROOT:\=/%/bin/bash.exe""; cd ""%PWD:\=/%""; CHERE_INVOKING=. exec ""%CYGWIN_ROOT:\=/%/bin/bash.exe"" -l -i; } 2>&1 | ""%CYGWIN_ROOT:\=/%/bin/tee.exe"" -a ""%PROJECT_LOG_FILE:\=/%"""
