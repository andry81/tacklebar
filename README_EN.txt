...
1. To install into a directory without a GUI do run the `_install.bat` with
   ...
   the script and configuration files.
...
2. At the end the installation script either does open an editor to edit the
   `_out/config/tacklebar/config.0.vars` file for correct values, or does open
   a merge tool to merge the file with one from a previous installation.
  ...
  different then, at the end of the installation you will be asked to merge
  values using a merge application from the `config.0.vars` file of the
  previous installation directory into the `config.0.vars` file of the new
  installation directory (basically a new installation directory has the same
  location as a previous one).

3. You can optionally install fonts from the `deploy/fonts` directory by run
   the `_install-fonts.bat` script.
...
For the Windows XP the initial code page in the `config.system.vars`
configuration file is taken from the registry on the moment of the
installation. You can change it:

(437 - OEM-US DOS code page)

>
mkdir c:\totalcmd\plugins\UTIL
_install.bat -chcp 437 c:\totalcmd\plugins\UTIL

>
_install-fonts.bat -chcp 437
...
NOTE:
  The sub list of variants here is not full and does change.
  You have to open all Button Bar menu items to lookup a related script usage.
...
CAUTION:
  ...
  line argument of an executable (an internal Windows command line parse
  issue).
...
Old implementation scripts may be called with the `winshell_call.vbs` + 
`call.vbs` scripts assistance.

See the `winshell_call.vbs` and `call.vbs` files header for the detailed help.
...
NOTE:
  Most of the scripts has `-+` and `--` flags to prevent flags and positional
  parameters from an accidental mix. This is useful if some flag parameters is
  passed as an external value and so can accidentally has a positional
  parameter.
...
NOTE:
  Starting Total Commander 11.57 you can prepend a command line with the
  asterisk `*` to run the command under Administrator.
...
2. Has no specific privilege rights restriction by the system and `cmd.exe`
   executable from there can be run under administrator user w/o any additional
   manipulations.

Using `mklink` command:

  https://github.com/andry81/contools/tree/HEAD/scripts/tools/ToolAdaptors/lnk/mklink_system64.bat

Using `mklink` command or `junction.exe` utility (Windows XP):

  https://github.com/andry81/contools/tree/HEAD/scripts/tools/ToolAdaptors/lnk/install_system64_link.bat
...
NOTE:
  In the Windows x64 will open `cmd.exe` which bitness will be dependent on
...
CAUTION:
  Method is obsoleted and moved into `contools.old` project.

NOTE:
  In non english version of the Windows instead of the "Administrator" you have
  to use a localized name.
